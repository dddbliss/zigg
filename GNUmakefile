...
.PHONY: run
run: $(IMAGE_NAME).iso
	qemu-system-x86_64 -M q35 -m 2G -cdrom $(IMAGE_NAME).iso -boot d -d -no-reboot

debug: $(IMAGE_NAME).iso
	qemu-system-x86_64 -M q35 -m 2G -cdrom $(IMAGE_NAME).iso -boot d -s -S
...
run-hdd: $(IMAGE_NAME).hdd
	qemu-system-x86_64 -M q35 -m 2G -device piix3-ide,id=ide -drive id=disk,file=$(IMAGE_NAME).hdd,format=raw,if=none -device ide-hd,drive=disk,bus=ide.0
